...
- `companion_*`
  ...
  - 今後の基本方針は「primary load の commit と同じ request で確定する」です。
  - 片側だけ後追い load で更新する構造は、表示ずれの温床として扱います。
...
- `apply_loaded_result()`
  ...
  - manga spread 有効時は、可能な限り companion も同じ request の結果としてここで同時 commit します。
