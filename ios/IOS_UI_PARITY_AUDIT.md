...
serviceだけが停止してscene復帰・delegate・dismantleのいずれも届かない場合に備え、通常のFiles pickerにも
アプリprocess所有の「閉じる」ボタンを重ねた。このボタンはDocument Managerの応答を待たずviewerへ戻し、
現在sourceと3x3入力を維持する。同じ復旧を2回行った後は3回目のpickerを事前遮断する。これはAppleの
`DUPLICATE_ITEM_IDENTIFIERS_IN_SECTION_SNAPSHOT` assertionやFavoritesデータを修復するものではない。
Simulatorでは復旧unit 5件、通常Cancel非遮断を含むSwift unit全70件、および復旧ボタンUI 1件が成功した。
